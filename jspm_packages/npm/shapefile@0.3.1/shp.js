...
function reader(filename) {
  ...
  function readRecord(callback) {
    if (!shapeType)
      return callback(new Error("must read header before reading records")), this;
    if (!(shapeType in readShape))
      return callback(new Error("unsupported shape type: " + shapeType)), this;
...
function readPoly(shapeType) {
  return function(record) {
    var box = [record.readDoubleLE(4), record.readDoubleLE(12), record.readDoubleLE(20), record.readDoubleLE(28)],
        ...
        i = 44,
        j;
    for (j = 0; j < numParts; ++j, i += 4)
      parts[j] = record.readInt32LE(i);

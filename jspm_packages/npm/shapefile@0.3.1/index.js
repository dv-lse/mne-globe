...
function reader(filename, options) {
  ...
  if (options)
    "encoding" in options && (encoding = options["encoding"]), "ignore-properties" in options && (ignoreProperties = !!options["ignore-properties"]);
  if (/\.shp$/.test(filename))
    filename = filename.substring(0, filename.length - 4);
  if (!ignoreProperties)
    dbfReader = dbf.reader(filename + ".dbf", encoding);

...
module.exports = function(topology, options) {
  var system = null,
      ...
      minimumArea;
  if (options)
    "coordinate-system" in options && (system = systems[options["coordinate-system"]]), "minimum-area" in options && (minimumArea = +options["minimum-area"]), "preserve-attached" in options && (preserveAttached = !!options["preserve-attached"]), "force-clockwise" in options && (forceClockwise = !!options["force-clockwise"]);
  if (forceClockwise)
    clockwise(topology, options);
  if (!(minimumArea > 0))
    minimumArea = Number.MIN_VALUE;
  if (preserveAttached) {
    var uniqueRingByArc = {},
        ringIndex = 0;
      ...
      polygon: function(arcs) {
        for (var i = 0,
          ...
          for (var ring = arcs[i],
            ...
            if (uniqueRing >= 0 && uniqueRing !== ringIndex)
              uniqueRingByArc[arc] = -1;
            else
              uniqueRingByArc[arc] = ringIndex;
          }
        }
      }
    });
  ...
  var filter = type({
    ...
    MultiPolygon: function(multiPolygon) {
      multiPolygon.arcs = multiPolygon.arcs.map(filterPolygon).filter(function(polygon) {
  ...
  function filterPolygon(arcs) {
    return arcs.length && filterExteriorRing(arcs[0]) ? [arcs.shift()].concat(arcs.filter(filterInteriorRing)) : null;
  }
